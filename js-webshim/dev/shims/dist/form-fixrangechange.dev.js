...
webshims.register('form-fixrangechange', function ($, webshims) {
  ...
  var rangeChange = {
    ...
    triggerInput: function triggerInput(e) {
      if (rangeChange.lastValue !== false && rangeChange.lastValue != e.target.value) {
        $(e.target).trigger('input');
      }
    },
    inputTeardown: function inputTeardown(e) {
      $(e.target).off('input', rangeChange.updateInputValue).off('blur', rangeChange.inputTeardown);
      rangeChange.lastValue = false;
    },
    inputSetup: function inputSetup(e) {
      if (e.target.type == 'range') {
        rangeChange.inputTeardown(e);
        rangeChange.lastValue = e.target.value;
        $(e.target).on('input', rangeChange.updateInputValue).on('blur', rangeChange.inputTeardown);
      }
    }
  };
  $.each([{
    ...
    rangeChange[obj.name + 'Block'] = function (e) {
      if (!rangeChange.blockElement && e.target.type == 'range') {
        ...
        $(rangeChange.blockElement).off('blur', rangeChange.trigger).on('blur', rangeChange.trigger);
        $(document.body).off(obj.evt, rangeChange[commit]).on(obj.evt, rangeChange[commit]);
      }
    };
  ...
  $(document.body || 'html').on({
    ...
    'keydown kepress': function keydownKepress(e) {

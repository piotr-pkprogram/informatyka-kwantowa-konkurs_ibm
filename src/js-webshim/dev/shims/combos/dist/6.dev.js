...
webshims.register('form-native-extend', function ($, webshims, window, doc, undefined, options) {
  ...
  webshims.addInputType = function (type, obj) {
    typeModels[type] = obj;
    runTest = true; //update validity of all implemented input types
  ...
  var testValidity = function testValidity(elem, init) {
    if (!elem && !runTest) {
      return;
    }
  ...
  if (overrideValidity) {
    ...
    if (doc.addEventListener) {
      ...
      var testPassValidity = function testPassValidity(e) {
        if (!('form' in e.target)) {
          return;
        }
...
webshims.register('form-number-date-api', function ($, webshims, window, document, undefined, options) {
  ...
  var nan = parseInt('NaN', 10),
    ...
    return $('<input type="' + type + '" />').prop('type') === type;
      ...
      getType = function getType(elem) {
    return (elem.getAttribute('type') || '').toLowerCase();
  },
      isDateTimePart = function isDateTimePart(string) {
    return string && !isNaN(string * 1);
  },
      addMinMaxNumberToCache = webshims.addMinMaxNumberToCache,
      addleadingZero = function addleadingZero(val, len) {
    val = '' + val;
    len = len - val.length;
  ...
  webshims.reflectProperties(['input'], ['max', 'min', 'step']); //IDLs and methods, that aren't part of constrain validation, but strongly tight to it
  ...
  var typeProtos = {
    number: {
      ...
      stepScaleFactor: 1,
      asNumber: function asNumber(str) {
    ...
    date: {
      ...
      asDate: function asDate(val, _noMismatch) {
      ...
      asNumber: function asNumber(str, _noMismatch) {
        var ret = nan;
    ...
    //		},
    time: {
      bad: function bad(val, _getParsed) {
        if (!val || !val.split || !/\d$/.test(val)) {
          return true;
        }
    ...
    'datetime-local': {
      ...
      dateToString: function dateToString(date, _getParsed) {

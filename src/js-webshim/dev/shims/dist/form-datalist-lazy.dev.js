...
webshims.register('form-datalist-lazy', function ($, webshims, window, document, undefined, options) {
  ...
  var getStoredOptions = function getStoredOptions(name) {
    if (!name) {
      return [];
    }
  ...
  var storeOptions = function storeOptions(name, val) {
    if (!name) {
      return;
    }
  ...
  $.extend(options.shadowListProto, {
    _lazyCreate: function _lazyCreate(opts) {
      var that = this;
      ...
      this.shadowList.on('mouseenter.datalistWidget mousedown.datalistWidget click.datalistWidget', 'li', function (e) {
      ...
      $(opts.input).attr({
        ...
        'keydown.datalistWidget': function keydownDatalistWidget(e) {
      ...
      $(this.datalist).off('updateDatalist.datalistWidget').on('updateDatalist.datalistWidget', $.proxy(this, '_resetListCached')).on('remove', function (e) {
        if (!e.originalEvent) {
          that.destroy();
        }
      });
    ...
    _resetListCached: function _resetListCached(e) {
      var that = this;
      var forceShow;
    ...
    updateListOptions: function updateListOptions(_forceShow) {
      ...
      $(this.input).removeAttr('aria-activedescendant').triggerHandler('datalistcreated', [{
        instance: this
      }]);
    ...
    getOptionContent: function getOptionContent(item) {
      var content;
      var args = [{
        instance: this,
        item: item
    ...
    setCompletedValue: function setCompletedValue(value, foundItem, length) {
      this.isCompleted = false;
      ...
      if (value.length == end) {
        ...
        $(input).callProp('setSelectionRange', [value.length, newValue.length]); //safari workaround || needs more investigation
    ...
    showHideOptions: function showHideOptions(_fromShowList) {
      ...
      var that = this; //first check prevent infinite loop, second creates simple lazy optimization
    ...
    addMark: function addMark(elem, item, prop, start, length) {
    ...
    removeMark: function removeMark(lis) {
      if (this.options.highlight) {
    ...
    changeValue: function changeValue(activeItem) {
      if (!activeItem[0]) {
        return;
      }
      ...
      if (newValue != oldValue) {
        $(this.input).prop('value', newValue).triggerHandler('updateInput');
    ...
    markItem: function markItem(index, doValue, items) {
      var activeItem;
      var goesUp;

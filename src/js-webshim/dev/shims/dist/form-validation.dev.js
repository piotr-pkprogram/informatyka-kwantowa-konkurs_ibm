...
webshims.register('form-validation', function ($, webshims, window, document, undefined, options) {
  ...
  var nonFormFilter = function nonFormFilter() {
  ...
  var returnValidityCause = function returnValidityCause(validity, elem) {
    var ret;
  ...
  var isInGroup = function isInGroup(name) {
    var ret;
  ...
  var switchValidityClass = function switchValidityClass(e) {
    if (!iVal.sel) {
      return;
    }
    ...
    var switchClass = function switchClass() {
      ...
      if (chromeBugs && (e.type == 'change' || webkitVersion < 537.36) && noFocusWidgets[shadowType] && $.find.matchesSelector(e.target, ':focus') || e.type == 'focusout' && elem.type == 'radio' && isInGroup(elem.name)) {
        return;
      }
      ...
      if (addClass) {
        shadowElem.addClass(addClass).removeClass(removeClass); //jQuery 1.6.1 IE9 bug (doubble trigger bug)
  ...
  var eachReset = function eachReset() {
  ...
  $(document.body || 'html').on(iVal.events + 'refreshvalidityui updatevalidation.webshims invalid', switchValidityClass).on('refreshvalidationui.webshims', function (e) {
    ...
    elems = elems.filter(iValClasses).removeAttr('aria-invalid').removeClass(iVal.errorClass + ' ' + iVal.successClass).getNativeElement().each(function () {
  ...
  var setRoot = function setRoot() {
    if (document.scrollingElement) {
      webshims.scrollRoot = $(document.scrollingElement);
    } else {
      webshims.scrollRoot = isWebkit || document.compatMode == 'BackCompat' ? $(document.body) : $(document.documentElement);
    }
  };
  ...
  var rtlReplace = function rtlReplace(ret) {
  ...
  $.extend(webshims.wsPopover, {
    isInElement: function isInElement(containers, contained) {
    ...
    show: function show(element) {
      ...
      var closeOnOutSide = function closeOnOutSide(e) {
      ...
      var delayedRepos = function delayedRepos(e) {
      ...
      $(document.body).on('focusin' + this.eventns + ' mousedown' + this.eventns, closeOnOutSide) //http://www.quirksmode.org/m/tests/eventdelegation2.html
      .children(':not(script), :not(iframe), :not(noscript)').on('mousedown' + this.eventns, closeOnOutSide);
    ...
    prepareFor: function prepareFor(element, visual) {
    ...
    clear: function clear() {
      ...
      $(document.body).off(this.eventns).children(':not(script), :not(iframe), :not(noscript)').off(this.eventns);
    ...
    hide: function hide() {
      ...
      var forceHide = function forceHide(e) {
  ...
  webshims.validityAlert = function () {
    ...
    api.element.addClass('validity-alert').attr({
      role: 'alert'
    });
    $.extend(api, {
      hideDelay: 5000,
      showFor: function showFor(elem, message, noFocusElem, noBubble) {
      ...
      getMessage: function getMessage(elem, message) {
        if (!message) {
  ...
  var errorBoxId = 0;
  webshims.errorbox = {
    create: function create(elem, fieldWrapper) {
      if (!fieldWrapper) {
    ...
    getFieldWrapper: function getFieldWrapper(elem) {
      var fieldWrapper;
    ...
    _createContentMessage: function () {
      var noValidate = function noValidate() {
      ...
      var deCamelCase = function deCamelCase(c) {
        return '-' + c.toLowerCase();
      };

      var getErrorName = function getErrorName(elem) {
    ...
    initIvalContentMessage: function initIvalContentMessage(elem) {
      var form;
    ...
    get: function get(elem, fieldWrapper) {
      if (!fieldWrapper) {
        fieldWrapper = this.getFieldWrapper(elem);
      }
      ...
      if ((type = _typeof(errorBox)) != 'object') {
        if (!errorBox) {
    ...
    addSuccess: function addSuccess(elem, fieldWrapper) {
    ...
    hideError: function hideError(elem, reset) {
    ...
    recheckInvalidInput: function recheckInvalidInput(input) {
      if (iVal.recheckDelay && iVal.recheckDelay > 90) {
        ...
        $(input).filter('input:not([type="checkbox"]):not([type="radio"]), textarea').off('.recheckinvalid').on('input.recheckinvalid', function () {
          clearTimeout(timer);
    ...
    showError: function showError(elem) {
      var fieldWrapper = this.getFieldWrapper(elem);
      ...
      if (box.message != message) {
        ...
        if (box.is('[hidden]') || box.css('display') == 'none') {
          ...
          box.css({
            display: 'none'
          }).removeAttr('hidden')[fx[iVal.fx].show]();
        }
      }
  ...
  $(document.body).on({
    'changedvaliditystate': function changedvaliditystate(e) {
      var form;
    ...
    'resetvalidityui.webshims': function resetvalidityuiWebshims(e) {
      var form;
    ...
    firstinvalid: function firstinvalid(e) {
      var form;
  ...
  function customFile() {
    ...
    $.data(this, 'wsCustomFile', {
      showSelected: showSelected
    });
    $('button:not(.ws-capture-button)', $module).attr('tabindex', '-1');
    $file.on('change.webshim', showSelected).each(showSelected).jProp('form').on('reset', function () {
      setTimeout(showSelected);
    });

    if (isIE) {
      $('<div class="ws-coverfile" />').insertAfter($file).on('click.webshim', function (e) {
        e.stopImmediatePropagation();
        $file.trigger('click');
      });
    }

...
webshims.register('form-validators', function ($, webshims, window, document, undefined, options) {
  ...
  (function () {
    ...
    var onEventTest = function onEventTest(e) {
    ...
    webshims.addCustomValidityRule = function () {
      ...
      var reTest = function reTest() {
        $(document.querySelectorAll(elemSels)).filter(noValidate).each(function () {
    ...
    webshims.refreshCustomValidityRules = function (elem) {
      ...
      var validity = data && $.prop(elem, 'validity') || {
        valid: 1
      };
      ...
      if (data && (customMismatchedRule || validity.valid)) {
        ...
        setMessage = function setMessage(message, errorType) {
          blockCustom = true;
    ...
    webshims.ready('forms form-validation', function () {
      ...
      setTimeout(function () {
        webshims.addReady(function (context, selfElement) {
          initTest = true;
          $(context.querySelectorAll(elemSels)).add(selfElement.filter(elemSels)).filter(noValidate).each(function () {
            testValidityRules(this);
          });
          formReady = true;
        });
  ...
  (function () {
    ...
    var getId = function getId(i, name) {
    ...
    addCustomValidityRule('grouprequired', function (elem, val, data) {
      ...
      if (!data.grouprequired.checkboxes) {
        ...
        data.grouprequired.checkboxes.off('click.groupRequired').on('click.groupRequired', function () {
    ...
    }, 'Please check one of these checkboxes.'); // based on https://sites.google.com/site/abapexamples/javascript/luhn-validation
    ...
    addCustomValidityRule('luhn', function (elem, value, data) {
      ...
      value = value.replace(/\-/g, ""); //if it's not numeric return true >- for invalid
    ...
    var dependentDefaults = {
      ...
      "prop": "value",
      //default: value||disabled	(last if "from-prop" is checked)
      "from-prop": "value",
      //default: value||checked (last if element checkbox or radio)
      "toggle": false
    };
    ...
    addCustomValidityRule('dependent', function (elem, val, data) {
      ...
      if (!data._init || !data.masterElement) {
        ...
        data = $.data(elem, 'dependentValidation', $.extend({
          _init: true
        }, dependentDefaults, data));
    ...
    addCustomValidityRule('ajaxvalidate', function (elem, val, data) {
      ...
      if (!data.remoteValidate) {
        if (typeof data.ajaxvalidate == 'string') {
          data.ajaxvalidate = {
            url: data.ajaxvalidate,
            depends: $([])
          };
        } else {
          data.ajaxvalidate.depends = data.ajaxvalidate.depends ? $(typeof data.ajaxvalidate.depends == 'string' && data.ajaxvalidate.depends.split(' ') || data.ajaxvalidate.depends).map(getId) : $([]);
        }
        ...
        var remoteValidate = {
          ...
          message: 'async',
          cache: {},
          update: function update(remoteData) {
            if (this.ajaxLoading) {
              ...
              $.ajax($.extend({
                ...
                url: opts.url,
                depData: remoteData,
                data: formCFG.fullRemoteForm || opts.fullForm ? $(elem).jProp('form').serializeArray() : remoteData,

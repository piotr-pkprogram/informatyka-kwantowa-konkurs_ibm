...
var swfmini = function () {
  var wasRemoved = function wasRemoved() {
  ...
  */
  ...
  webshims.loader.addModule('swfmini-embed', {
    d: ['swfmini']
  });
  var loadEmbed = hasPlayerVersion('9.0.0') ? function () {
  ...
  return {
    ...
    */
    ...
    createSWF: function (_createSWF) {
      ...
      return createSWF;
    }(function (attObj, parObj, replaceElemIdStr) {
      if (ua.w3) {
...
webshims.register('form-core', function ($, webshims, window, document, undefined, options) {
  ...
  var extendSels = function extendSels() {
    var matches, matchesOverride;
    var exp = $.expr[":"];
    var rElementsGroup = /^(?:form|fieldset)$/i;

    var hasInvalid = function hasInvalid(elem) {
      var ret = false;
    ...
    $.extend(exp, {
      "valid-element": function validElement(elem) {
        return rElementsGroup.test(elem.nodeName || '') ? !hasInvalid(elem) : !!($.prop(elem, 'willValidate') && isValid(elem));
      },
      "invalid-element": function invalidElement(elem) {
        return rElementsGroup.test(elem.nodeName || '') ? hasInvalid(elem) : !!($.prop(elem, 'willValidate') && !isValid(elem));
      },
      "required-element": function requiredElement(elem) {
        return !!($.prop(elem, 'willValidate') && $.prop(elem, 'required'));
      },
      "user-error": function userError(elem) {
        return $.prop(elem, 'willValidate') && $(elem).getShadowElement().hasClass(options.iVal.errorClass || 'user-error');
      },
      "optional-element": function optionalElement(elem) {
  ...
  var lazyLoadProxy = function lazyLoadProxy(obj, fn, args) {
    lazyLoad();
  ...
  addModule('form-validation', $.extend({
    d: ['form-message']
  }, formExtras));
  ...
  webshims.wsPopover = {
    ...
    bindElement: function bindElement() {
      ...
      var stopBlur = function stopBlur() {
        that.stopBlur = false;
      };
  ...
  webshims.getContentValidationMessage = function (elem, validity, key) {
    ...
    if (key && message[key]) {
      ...
      validity = validity || $.prop(elem, 'validity') || {
        valid: 1
      };
...
webshims.register('form-shim-extend', function ($, webshims, window, document, undefined, options) {
  ...
  webshims.inputTypes = webshims.inputTypes || {}; //some helper-functions
  ...
  var typeModels = webshims.inputTypes,
    ...
    return webshims.inputTypes[type] ? type : elem.type;
      ...
      cacheType = function cacheType(cache, input) {
    if (!('type' in cache)) {
  ...
  var isPlaceholderOptionSelected = function isPlaceholderOptionSelected(select) {
  ...
  var getGroupElements = function getGroupElements(elem) {
    elem = $(elem);
    var name, form;
    var ret = emptyJ;
  ...
  var lengthTypes = $.extend({
    textarea: 1
  }, patternTypes);
  var validityRules = {
    valueMissing: function valueMissing(input, val, cache) {
    ...
    patternMismatch: function patternMismatch(input, val, cache) {
      var i;
      var ret = false;
  ...
  $.event.special.invalid = {
    ...
    setup: function setup() {
      ...
      $(form).data('invalidEventShim', true).on('submit', $.event.special.invalid.handler);
  ...
  var baseCheckValidity = function baseCheckValidity(elem, type) {
    var e,
    ...
    (function () {
      var noInputTriggerEvts = {
        ...
        change: 1
          ...
          observe = function observe(input) {
        var timer,
            focusedin = true,
  ...
  webshims.addReady(function (context, contextElem) {
    ...
    $('form', context).add(contextElem.filter('form')).on('invalid', $.noop);
    setTimeout(function () {
      var focusElem;
  ...
  [{
    ...
    var changeSubmitter = function changeSubmitter() {
      var elem = this;
...
webshims.register('form-message', function ($, webshims, window, document, undefined, options) {
  ...
  if (_typeof(validityMessages.de.rangeOverflow) == 'object') {
    ['date', 'time', 'datetime-local', 'month'].forEach(function (type) {
  ...
  var getMessageFromObj = function getMessageFromObj(message, elem) {
  ...
  var toLocale = function () {
    ...
    var _toLocale = function _toLocale(val, elem, attr) {
      var type, widget;
...
(function (webshims) {
  ...
  webshims.register('mediaelement-core', function ($, webshims, window, document, undefined, options) {
    ...
    var getSrcObj = function getSrcObj(elem, nodeName) {
      elem = $(elem);
      var src = {
        src: elem.attr('src') || '',
        elem: elem,
        srcProp: elem.prop('src')
      };
      var tmp;
      ...
      if (tmp) {
        ...
        if (!nodeName) {
          ...
          if (nodeName == 'source') {
            nodeName = (elem.closest('video, audio')[0] || {
              nodeName: 'video'
    ...
    var loadTrackUi = function loadTrackUi() {
    ...
    var loadThird = function loadThird() {
      if (hasSwf) {
        loadSwf();
      } else {
        loadYt();
    ...
    mediaelement.canNativePlaySrces = function (mediaElem, srces) {
      ...
      if (hasNative) {
        mediaElem = $(mediaElem);
        var nodeName = (mediaElem[0].nodeName || '').toLowerCase();
        var nativeCanPlay = (nativeCanPlayType[nodeName] || {
          prop: {
            _supvalue: false
          }
        }).prop._supvalue || mediaElem[0].canPlayType;
    ...
    var getRemoveEmptyType = function getRemoveEmptyType() {
    ...
    var activate = {
      "native": function native(elem, src, data) {
    ...
    var stepSources = function stepSources(elem, data, srces) {
      var i, src;
      var testOrder = [{
    ...
    var selectSource = function selectSource(elem, data) {
    ...
    var initMediaElements = function initMediaElements() {
      var testFixMedia = function testFixMedia() {
      ...
      webshims.ready('dom-support', function () {
        ...
        webshims.addReady(function (context, insertedElement) {
          var media = $('video, audio', context).add(insertedElement.filter('video, audio')).each(testFixMedia);
      ...
      if (hasNative && !handleMedia) {
        webshims.addReady(function (context, insertedElement) {
          if (!handleMedia) {
            $('video, audio', context).add(insertedElement.filter('video, audio')).each(function () {

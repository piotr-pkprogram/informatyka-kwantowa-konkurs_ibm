...
(function ($) {
  if (webshims.support.texttrackapi && document.addEventListener) {
    ...
    var changeApi = function changeApi() {
    ...
    var detectTrackError = function detectTrackError() {
...
webshims.register('track-ui', function ($, webshims, window, document, undefined) {
  ...
  var support = webshims.support; //descriptions are not really shown, but they are inserted into the dom
  ...
  var usesNativeTrack = function usesNativeTrack() {
  ...
  var trackDisplay = {
    update: function update(baseData, media) {
    ...
    showCues: function showCues(baseData) {
    ...
    addEvents: function addEvents(baseData, media) {
      if (options.positionDisplay) {
        var timer;

        var positionDisplay = function positionDisplay(_force) {
        ...
        var delayed = function delayed(e) {
          clearTimeout(timer);
  ...
  function triggerCueEvent(cue, type, baseData, media, trackIndex) {
    ...
    if (trackElem) {
      compareTrack = (webshims.data(trackElem, 'trackData') || {
        track: $.prop(trackElem, 'track')
      }).track;
  ...
  var filterTrackImplementation = function filterTrackImplementation() {
    return webshims.implement(this, 'trackui');
  };

  var implementTrackUi = function implementTrackUi() {
    ...
    var reCheck = function reCheck() {
      recheckI++; //if recheckI is over 5 video might be paused, stalled or waiting,
    ...
    var getDisplayCues = function getDisplayCues(e) {
      var track, time;
    ...
    var invalidateTrackElems = function invalidateTrackElems() {
      if (baseData && baseData.trackElements) {
        delete baseData.trackElements;
      }
    };

    var onUpdatCues = function onUpdatCues(e) {
    ...
    var addTrackView = function addTrackView() {
      if (!trackList) {
      ...
      $([trackList]).off('.trackview').on('change.trackview addtrack.trackview removetrack.trackview', onUpdatCues);
      elem.off('.trackview').on('emptied.trackview', invalidateTrackElems).on('play.trackview playing.trackview updatetrackdisplay.trackview seeked.trackview', onUpdatCues).on('timeupdate.trackview', getDisplayCues);
    };
  ...
  if (usesNativeTrack()) {
    (function () {
      ...
      var triggerDisplayUpdate = function triggerDisplayUpdate(elem) {
        block = true;
        setTimeout(function () {
      ...
      var createUpdateFn = function createUpdateFn(nodeName, prop, type) {
  ...
  webshims.addReady(function (context, insertedElement) {
    $('video, audio', context).add(insertedElement.filter('video, audio')).filter(filterTrackImplementation).each(implementTrackUi);
  });
});
...
webshims.register('mediacontrols-lazy', function ($, webshims, window, doc, undefined) {
  ...
  var onSliderReady = function onSliderReady(fn) {
    loadRange();
  ...
  var getTrackMenu = function getTrackMenu(tracks) {
  ...
  $.extend(true, plugins, {
    useractivity: {
      _create: function _create(empty1, empty2, base) {
        base.on({
    ...
    'play-pause': {
      ...
      _create: function _create(control, media) {
        var textFn = $.jme.getButtonText(control, [this[pseudoClasses].play, this[pseudoClasses].pause]);
        media.on('play playing ended pause updateJMEState', function (e) {
          var state = e.type;
    ...
    'mute-unmute': {
      ...
      _create: function _create(control, media, base) {
        ...
        media.on('volumechange updateJMEState', function (e) {
    ...
    'jme-media-overlay': {
      _create: function _create(control, media, base) {
        ...
        base.on({
          'touchstart touchend mousedown click mouseover': function touchstartTouchendMousedownClickMouseover(e) {
            var delay = 500;
            stopFocus = true;
    ...
    'time-slider': {
      ...
      _create: function _create(control, media, base) {
        ...
        var createFn = function createFn() {
          var time, durationChange, api, timeShow, wasPaused, _hideTime;
          ...
          _hideTime = function hideTime() {
            setTimeout(function () {
              timeShow.removeClass('show-time-select');
              control.off('.jmetimeselect');

              if (document.removeEventListener) {
                document.removeEventListener('touchend', _hideTime, true);
              }
            });
          };
            ...
            'mouseenter touchstart': function mouseenterTouchstart(e) {
              if (hasDuration && e.type != 'touchstart' || e.originalEvent && e.originalEvent.touches && e.originalEvent.touches.length == 1) {
                ...
                if (document.addEventListener) {
                  document.addEventListener('touchend', _hideTime, true);
                }

                control.off('.jmetimeselect').on('mousemove.jmetimeselect touchmove.jmetimeselect', function (e) {
                  $.fn.rangeUI.normalizeTouch(e);
                  posLeft(e.pageX);
                });
              }
            },
            'mouseleave touchend': _hideTime
          });
          media.on({
            timeupdate: time.get,
    ...
    'duration-display': {
      _create: function _create(control, media, base, options) {
        ...
        control.on('updatetimeformat', showDuration).jmeProp('format', options.format);
      }
    },
    'currenttime-display': {
      _create: function _create(control, media, base, options) {
        ...
        var showTime = function showTime(e) {
        ...
        control.on('updatetimeformat', showTime).jmeProp('format', options.format);
      }
    },
    'poster-display': {
      _create: function _create(control, media) {
         ...
         */
        var updatePoster = function updatePoster() {
    ...
    fullscreen: {
      ...
      _create: function _create(control, media, base) {
        ...
        var addDoubbleClick = function addDoubbleClick() {
          $(base.data('jme').controlElements).filter('.jme-media-overlay').off('.dblfullscreen').on('dblclick.dblfullscreen', function (e) {
    ...
    chapters: {
      _create: function _create(control, media, base) {
        ...
        webshims.ready('track', function () {
          ...
          var timedPreload = function timedPreload() {
            clearTimeout(preloadTimer);
            preloadTimer = setTimeout(setPreload, 999);
          };

          var setPreload = function setPreload() {
            var preload;
          ...
          var buildMenu = function buildMenu(currentTrack, chapterList) {
            if ($bar) {
              $bar.remove();
              $bar = null;
            }
            ...
            if (currentTrack && chapterList.length) {
              ...
              var text = currentTrack.label || plugin.text; //$bar = chapterList.map(createChapterBar);
      ...
      _create: function _create(control, media, base) {
        ...
        webshims.ready('track', function () {
          ...
          function createSubtitleMenu(menu) {
            ...
            if (!menuObj) {
              menuClick = function menuClick(index, button) {
          ...
          if (!textTracks) {
            ...
            updateTrackMenu();
            $([textTracks]).on('addtrack removetrack', throttledUpdate).on('change', throttledUpdateMode);
  ...
  var trackFilters = {
    chapters: function chapters(track) {
      return track.kind == 'chapters';
    },
    notDisabled: function notDisabled(track) {
      return track.mode != 'disabled';
    },
    activeLang: function activeLang(track) {
      return track.language == webshims.activeLang();
    },
    activePartialLang: function activePartialLang(track) {
  ...
  $.jme.defineMethod('activateTrack', function (track, success) {
    ...
    var checkTrackState = function checkTrackState() {
      clearTimeout(timer);
  ...
  var formatTime = function formatTime(sec, format) {
    var data;
  ...
  $.jme.defineMethod('formatTime', formatTime); //taken from http://johndyer.name/native-fullscreen-javascript-api-plus-jquery-plugin/
  ...
  $.jme.fullscreen = function () {
    ...
    fullScreenApi.requestFullWindow = fullScreenApi.requestFullScreen; // update methods to do something useful
  ...
  $.jme.defineProp('fullscreen', {
    set: function set(elem, value) {
      ...
      if (value) {
        ...
        $(document).off('.jmefullscreen').on('keydown.jmefullscreen', function (e) {
          if (e.keyCode == 27) {
        ...
        if ($.jme.fullscreen.supportsFullScreen) {
          $(document).on($.jme.fullscreen.eventName + '.jmefullscreen', function (e) {
  ...
  $.jme.registerPlugin('buffer-progress', {
    _create: function _create(control, media, base, options) {
      ...
      var drawBufferProgress = function drawBufferProgress() {
      ...
      media.on({
        ...
        'seeked seeking updateprogress': function seekedSeekingUpdateprogress(e) {
          clearTimeout(progressTimer);
  ...
  $.jme.ButtonMenu = function (button, menu, clickHandler) {
    ...
    this.button.wsTouchClick(this.toggle).on('keydown', function (e) {
  ...
  $.jme.ButtonMenu.prototype = {
    addMenu: function addMenu(menu) {
      ...
      if (this.clickHandler) {
        ...
        this.menu.attr('role', 'menu').on('keydown', this.keyIndex).wsTouchClick('button', this._buttonClick);
      }
    },
    _closeFocusOut: function _closeFocusOut() {
      ...
      var stopFocusOut = function stopFocusOut() {
        clearTimeout(timer);
        setTimeout(function () {
          clearTimeout(timer);
        }, 9);
      ...
      this.menu.parent().on('focusin mousedown click touchend', stopFocusOut).on('focusout', function (e) {
  ...
  (function () {
    var activity = {
      add: function add(elem, cfg, name) {
        ...
        if (!data.bound) {
          jElm.on('mouseleave.jmeuseractivity', setInactive).on('touchend.jmeuseractivity setuseractive.jmeuseractivity mousemove.jmeuseractivity focusin.jmeuseractivity mouseenter.jmeuseractivity keydown.jmeuseractivity keyup.jmeuseractivity mousedown.jmeuseractivity', setActive);
          data.bound = true;
        }
      ...
      remove: function remove(elem, name) {
        var data = $.data(elem, 'jmeuseractivity') || $.data(elem, 'jmeuseractivity', {
          idletime: 2500,
          idle: true,
          trigger: {}

...
webshim.register('es6', function ($, webshim, window, document, undefined) {
  ...
  var isCallableWithoutNew = function isCallableWithoutNew(func) {
    try {
      func();
    } catch (e) {
      return false;
  ...
  var main = function main() {
    ...
    var _create = Object.create || function (prototype, properties) {
      function Type() {}
    ...
    var $iterator$ = typeof Symbol === 'function' && Symbol.iterator || '_es6shim_iterator_'; // Firefox ships a partial implementation using the name @@iterator.
    ...
    var isArguments = function isArguments(value) {
      ...
      if (!result) {
        result = str !== '[object Array]' && value !== null && _typeof(value) === 'object' && typeof value.length === 'number' && value.length >= 0 && _toString.call(value.callee) === '[object Function]';
      }

      return result;
    };

    var emulateES6construct = function emulateES6construct(o) {
      if (!ES.TypeIsObject(o)) throw new TypeError('bad object'); // es5 approximation to es6 subclass semantics: in es6, 'new Foo'
    ...
    var ES = {
      CheckObjectCoercible: function CheckObjectCoercible(x, optMessage) {
        /* jshint eqnull:true */
        if (x == null) throw new TypeError(optMessage || 'Cannot call method on ' + x);
        return x;
      },
      ...
      Construct: function Construct(C, args) {
    ...
    var numberConversion = function () {
      ...
      function unpackIEEE754(bytes, ebits, fbits) {
        ...
        f = parseInt(str.slice(1 + ebits), 2); // Produce number
      ...
      var conversions = {
        toFloat32: function toFloat32(num) {
    ...
    var StringShims = {
      ...
      repeat: function () {
        var repeat = function repeat(s, times) {
          if (times < 1) return '';
      ...
      contains: function contains(searchString) {
        var position = arguments.length > 1 ? arguments[1] : undefined; // Somehow this trick makes method 100% compat with the spec.
    ...
    defineProperties(Array, {
      from: function from(iterable) {
        ...
        var usingIterator = ES.IsIterable(list); // does the spec really mean that Arrays should use ArrayIterator?
    ...
    var MathShims = {
      ...
      hypot: function hypot(x, y) {
        ...
        Array.prototype.every.call(arguments, function (arg) {
          var num = Number(arg);
          if (Number.isNaN(num)) anyNaN = true;else if (num === Infinity || num === -Infinity) anyInfinity = true;else if (num !== 0) allZero = false;
      ...
      imul: function imul(x, y) {
        ...
        var al = x & 0xffff;
        var bh = y >>> 16 & 0xffff;
        var bl = y & 0xffff; // the shift by 0 fixes the sign on the high part
    ...
    var PromiseShim = function () {
      ...
      var PromiseCapability = function PromiseCapability(C) {
        ...
        capability.promise = ES.Construct(C, [resolver]); // see https://bugs.ecmascript.org/show_bug.cgi?id=2478
      ...
      var triggerPromiseReactions = function triggerPromiseReactions(reactions, x) {
      ...
      var updatePromiseFromPotentialThenable = function updatePromiseFromPotentialThenable(x, capability) {
      ...
      var promiseResolutionHandler = function promiseResolutionHandler(promise, onFulfilled, onRejected) {
        return function (x) {
          if (x === promise) {
      ...
      defineProperties(Promise, {
        '@@create': function create(obj) {
          var constructor = this; // AllocatePromise
          // The `obj` parameter is a hack we use for es5
          // compatibility.

          var prototype = constructor.prototype || Promise$prototype;
          obj = obj || _create(prototype);
          defineProperties(obj, {
            _status: undefined,
            _result: undefined,
      ...
      Promise.prototype.then = function (onFulfilled, onRejected) {
        ...
        if (!ES.IsCallable(onFulfilled)) {
          onFulfilled = function onFulfilled(x) {
            return x;
          };
        }

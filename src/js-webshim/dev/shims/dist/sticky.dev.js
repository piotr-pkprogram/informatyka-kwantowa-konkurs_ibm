...
webshim.register('sticky', function ($, webshim, window, document, undefined, featureOptions) {
  ...
  var getWinScroll = function () {
    ...
    return prop in window ? function () {
      return window[prop];
  ...
  var stickyMixin = {
    getPosition: function getPosition() {
      if (!this.isSticky) {
        ...
        if ((this.position.top != 'auto' && this.position.bottom != 'auto' || this.position.top == 'auto' && this.position.bottom == 'auto') && this.$el.css('position') == 'static') {
          this.position = $.swap(this.$el[0], {
            position: 'absolute'
          }, getPos);
        }
    ...
    commonAddEvents: function commonAddEvents() {
      ...
      if (media && media.addListener) {
        enableDisable = function enableDisable() {
    ...
    setSticky: function setSticky() {
      ...
      this.setTdWidth();
      this.$placeholder.insertAfter(this.$el).outerHeight(this.stickyData.outerHeight, true).outerWidth(this.stickyData.outerWidth);
      this.isSticky = true;
  ...
  $.extend(Sticky.prototype, stickyMixin, {
    addEvents: function addEvents() {
      ...
      $window.on('scroll' + this.evtid, function () {
    ...
    updateDimension: function updateDimension(fromPos) {
      if (this.isSticky) {
        this.removeSticky();
      }
    ...
    updatePos: function updatePos(fromDimension) {
      ...
      if (this.ankered == 'top') {
        ...
        if (this.stickyData.bottom > offset && offset + 9 >= this.parentData.top) {
          shouldSticky = true;
  ...
  $.extend(StickyParent.prototype, stickyMixin, {
    addEvents: function addEvents() {
      var that = this;
      this.commonAddEvents();
      this.$parent.on('scroll' + this.evtid, function () {
    ...
    updatePos: function updatePos(fromDimension) {
      ...
      if (shouldSticky) {
        if (!this.isSticky) {
          ...
          this.setSticky();
          $window.off('scroll' + this.evtid, this.updatePos2).on('scroll' + this.evtid, this.updatePos2);
    ...
    updatePos2: function updatePos2(init) {
  ...
  var _loadDomSupport = function loadDomSupport() {
    _loadDomSupport = $.noop;
  ...
  if (!support.sticky && support.fixed) {
    ...
    var createUpdateDomSearch = function createUpdateDomSearch(media, sels) {
      var i, created, elems;
      var updated = [];
      ...
      if (!selectors[media]) {
        selectors[media] = {
          sels: {},
          string: '',
          fn: function fn(context, insertedElement) {
    ...
    if (featureOptions.parseCSS) {
      if (window.Polyfill && Polyfill.prototype && Polyfill.prototype.doMatched) {
        var onEnableRule = function onEnableRule(rule) {

...
(function (factory) {
  ...
  var addAsync = function addAsync() {
  ...
  window.webshims = {
    ...
    activeLang: function activeLang(lang) {
      addAsync();
      window.asyncWebshims.lang = lang;
    },
    polyfill: function polyfill(features) {
      addAsync();
      window.asyncWebshims.polyfill = features;
    },
    _curScript: function () {
      var scripts, i, scriptUrl, match, regUrl; //modern browsers: Chrome 29+, Firefox 4+

      var currentScript = document.currentScript; //in debug mode remove result to fully test fallback in all browsers
      ...
      if (!currentScript) {
        ...
        scripts = document.scripts || document.getElementsByTagName('script'); //get script by URL or by readyState == 'interactive' (readySate is supported in IE10-)
  ...
  var addSource = function addSource(text) {
    return text + "\n//# sourceURL=" + this.url;
  };

  var getAutoEnhance = function getAutoEnhance(prop) {
  ...
  $.extend(webshims, {
    ...
    cfg: {
      ...
      loadScript: function loadScript(src, success) {
    ...
    features: {},
    featureList: [],
    setOptions: function setOptions(name, opts) {
    ...
    addPolyfill: function addPolyfill(name, cfg) {
      cfg = cfg || {};
    ...
    _polyfill: function _polyfill(features) {
      var toLoadFeatures = [];
    ...
    reTest: function () {
      ...
      var reTest = function reTest(i, name) {
    ...
    isReady: function isReady(name, _set) {
      name = name + 'Ready';
    ...
    ready: function ready(events, fn
    /*, _created*/
    ) {
     ...
     */
    capturingEvents: function capturingEvents(names, _maybePrevented) {
     ...
     */
    loader: {
      addModule: function addModule(name, ext) {
        modules[name] = ext;
      ...
      loadList: function () {
        ...
        var loadScript = function loadScript(src, names) {
          if (typeof names == 'string') {
            names = [names];
          }
        ...
        var noNeedToLoad = function noNeedToLoad(name, list) {
        ...
        var setDependencies = function setDependencies(module, list) {
          if (module.d && module.d.length) {
            var addDependency = function addDependency(i, dependency) {
        ...
        return function (list) {
          ...
          var loadCombo = function loadCombo(j, combo) {
            foundCombo = combo;
      ...
      makePath: function makePath(src) {
  ...
  (function () {
    ...
    var eachTrigger = function eachTrigger() {
      if (this.nodeType == 1) {
    ...
    $.extend(webshims, {
      addReady: function addReady(fn) {
        var readyFn = function readyFn(context, elem) {
      ...
      triggerDomUpdate: function triggerDomUpdate(context) {
  ...
  addPolyfill('es6', {
    test: !!(Math.imul && Number.MIN_SAFE_INTEGER && Object.is && window.Promise && Promise.all),
    // && window.Map && Map.prototype && typeof Map.prototype.forEach !== 'function' && window.Set
    d: ['es5']
  }); //>
  //<geolocation
  ...
  (function () {
    ...
    var _initialFormTest = function initialFormTest() {
      var tmp, fieldset;
      var testValue = '1(';
      ...
      _initialFormTest = $.noop;
      return false;
    };

    var typeTest = function typeTest(o) {
      var ret = true;
    ...
    addPolyfill('form-number-date-ui', {
      ...
      test: function test() {
        ...
        _initialFormTest(); //input widgets on old androids can't be trusted
  ...
  (function () {
    ...
    $.each([['mediacontrols', {
      ...
      c: [98, 97]
    }], ['alternate-media']], function (i, plugin) {

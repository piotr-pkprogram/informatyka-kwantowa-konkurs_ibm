...
(function ($) {
  (function () {
    ...
    function handler(event) {
      ...
      event.deltaX = deltaX;
      event.deltaY = deltaY;
      event.deltaFactor = lowestDelta; // Add event and delta to the front of the arguments
  ...
  (function () {
    ...
    $.event.special.mwheelIntent = {
      ...
      teardown: function teardown() {
        $(this).off('mousewheel', $.event.special.mwheelIntent.handler).off('mouseleave', unsetPos);
        return true;
      },
    ...
    $(function () {
      body = doc.body; //assume that document is always scrollable, doesn't hurt if not
  ...
  (function () {
    ...
    var removeTimer = function removeTimer(elem, full) {
    ...
    $.event.special.mousepress = {
      setup: function setup() {
        ...
        $(this).on('mousedown.mousepressext', function (e) {
          ...
          var startIntervall = function startIntervall(delay) {
            var steps = 0;
            removeTimer(elem);
      ...
      handler: function handler(elem, e) {
...
webshims.register('forms-picker', function ($, webshims, window, document, undefined, options) {
  ...
  var _setFocus = function _setFocus(element, _noFocus) {
    element = $(element || this.activeButton);
    this.activeButton.attr({
      tabindex: '-1',
      'aria-selected': 'false'
    });
    this.activeButton = element.attr({
      tabindex: '0',
      'aria-selected': 'true'
    });
  ...
  var _initialFocus = function _initialFocus() {
    var sel;
  ...
  var getMonthNameHTML = function getMonthNameHTML(index, year, prefix) {
  ...
  var widgetProtos = {
    _addBindings: function _addBindings() {
      ...
      var initChangeEvents = function initChangeEvents() {
      ...
      var preventBlur = function preventBlur(e) {
      ...
      var elementEvts = {
        ...
        ws__focus: function ws__focus(e) {
          if (!isFocused) {
            initChangeEvents();
            isFocused = this;
          }
        ...
        'ws__input keydown keypress': function () {
          ...
          var releaseTab = function releaseTab() {
            if (isStopped === true) {
              isStopped = 'semi';
      ...
      var mouseDownInit = function mouseDownInit() {
      ...
      if (steps[this.type]) {
        ...
        $(this.buttonWrapper).on('mousepressstart mousepressend', '.step-up, .step-down', function (e) {
          var fn = 'removeClass';
    ...
    _getSelectionEnd: function _getSelectionEnd(val) {
  ...
  picker.getDayList = function (value, data) {
    ...
    for (j = 0; j < size; j++) {
      ...
      str.push('<button data-action="setMonthList"' + (o.minView >= 2 ? ' disabled="" ' : '') + ' value="' + dateArray.date + '" tabindex="-1">' + getMonthNameHTML(dateArray[1] * 1 - 1, dateArray[0]) + '</button>');
  ...
  picker.getTimeList = function (value, data) {
    ...
    for (; i <= len; i++) {
      ...
      hVal = valPrefix ? valPrefix + iVal : iVal;
  ...
  (function () {
    var retNames = function retNames(name) {
      return 'get' + name + 'List';
    };

    var retSetNames = function retSetNames(name) {
    ...
    var setDirButtons = function setDirButtons(content, popover, dir) {
      if (content[dir]) {
        //set content and idl attribute (content for css + idl for IE8-) 
        popover[dir + 'Element'].attr(content[dir]).prop({
          disabled: false
        }).prop(content[dir]);
      } else {
        popover[dir + 'Element'].removeAttr('data-action').prop({
          disabled: true
        });
      }
    };
        ...
        $.each(getNames, function (i, item) {
          if (i >= startAt) {
            ...
            if (values.length < 2 || content.enabled > 1 || content.prev || content.next || stops[data.type] === names[i]) {
              popover.element.attr({
                'data-currentview': setNames[i]
              }).addClass('ws-size-' + o.size).data('pickercontent', {
                data: data,
                content: content,
                values: values
              });
              ...
              popover.element.filter('[data-vertical="bottom"]').triggerHandler('pospopover');
              return false;
            }
          }
        });
  ...
  picker.commonDateInit = function (data, popover) {
    ...
    var actionfn = function actionfn(e) {
    ...
    var generateList = function generateList(o, max, min) {
      var options = [];
      var label = '';
      var labelId = '';
      ...
      $.each(o.options[0], function (val, label) {
        var disabled = picker.isInRange(val.split('-'), o.maxS, o.minS) ? '' : ' disabled="" ';
      ...
      if (options.length) {
        ...
        new webshims.ListBox($('<div class="ws-options">' + label + '<ul role="listbox" ' + labelId + '>' + options.join('') + '</div>').insertAfter(popover.bodyElement)[0], popover, {
          noFocus: true
        });
      }
    };
    ...
    var updateContent = function updateContent() {
      ...
      if (popover.isDirty) {
        ...
        popover.nextElement.attr({
          'aria-label': curCfg.date.nextText
        });
        popover.prevElement.attr({
          'aria-label': curCfg.date.prevText
        });
        popover.yearNextElement.attr({
          'aria-label': curCfg.date.nextText
        });
        popover.yearPrevElement.attr({
          'aria-label': curCfg.date.prevText
        });
    ...
    popover.contentElement.wsTouchClick('button[data-action]', actionfn).on('change', 'select[data-action]', actionfn);

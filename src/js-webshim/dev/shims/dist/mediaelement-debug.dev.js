...
(function (webshim, $) {
  ...
  function runMediaTest(src, container, provider, infos) {
    ...
    var $media = $(document.createElement(infos.nodeName)).attr({
      src: src.src,
      'data-type': container,
      'controls': 'controls',
      preload: 'none'
    ...
    var runEnded = function runEnded(e) {
    ...
    var resolve = function resolve(e) {
        ...
        if (e.type == 'loadedmetadata') {
          ...
          if (!src.decode[provider] || $.isEmptyObject(src.decode[provider])) {
            src.decode[provider] = {
              ...
              width: $media.prop('videoWidth') //todo at test for seekable
    ...
    if (!$container.length) {
      $container = $('<div id="wsmediatestcontainer" />').css({
        position: 'fixed',
        top: 0,
        left: 0,
        right: 0,
    ...
    $media.on('mediaerror loadedmetadata', resolve).appendTo($element);
  ...
  function runDecodeTest(src, infos) {
    ...
    if (hasFlash && !/youtube|rtmp/i.test(type) && mediaelement.swfMimeTypes.indexOf(type) != -1) {
      ...
      src.decode.swf = {
        success: false,
        notsupported: type != 'video/youtube'
      };
    }
  ...
  function getMediaInfo(elem) {
    ...
    var initTests = function initTests() {
      runTests(infos);
    };
  ...
  var timedMediaInfo = function timedMediaInfo(i) {
    var elem = this;
    setTimeout(function () {
      getMediaInfo(elem);
    }, i * 100);
  ...
  webshim.addReady(function (context, $insertedElement) {
    $('video, audio', context).add($insertedElement.filter('video, audio')).each(timedMediaInfo);
  });

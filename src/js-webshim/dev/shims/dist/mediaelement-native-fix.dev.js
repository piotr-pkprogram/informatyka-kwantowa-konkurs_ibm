...
webshims.register('mediaelement-native-fix', function ($, webshims, window, document, undefined) {
  ...
  var fixBuffered = function () {
    ...
    var getBufferedData = function getBufferedData(elem) {
    ...
    var loadProgessListener = function loadProgessListener(e) {
      e = e.originalEvent;
    ...
    var removeProgress = function removeProgress(e) {
    ...
    return function () {
      $(this).off('load progress', loadProgessListener).off('emptied', removeProgress).on('load progress', loadProgessListener).on('emptied', removeProgress);
    };
  }();
  ...
  (function () {
    ...
    if (!('preload' in videoElem) && 'autobuffer' in videoElem) {
      ...
      webshims.onNodeNamesPropertyModify(['audio', 'video'], ['preload'], {
        set: function set(value, boolValue, curType) {
  ...
  var fixProgressEvent = function () {
    ...
    return function () {
      ...
      var getBufferedString = function getBufferedString() {
      ...
      var testBuffer = function testBuffer() {
      ...
      $(this).data('fixedMediaProgress', true).on({
        'play loadstart progress': function playLoadstartProgress(e) {
          if (e.type == 'progress') {
        ...
        'emptied stalled mediaerror abort suspend': function emptiedStalledMediaerrorAbortSuspend(e) {
          if (e.type == 'emptied') {
            lastBuffered = false;
          }
  ...
  webshims.addReady(function (context, insertedElement) {
    $('video, audio', context).add(insertedElement.filter('video, audio')).each(fixBuffered).each(fixProgressEvent);
  });
});
...
webshim.register('filereader-xhr', function ($, webshim, window, document, undefined, featureOptions) {
  ...
  var loadMoxie = function loadMoxie() {
    webshim.loader.loadList(['moxie']);
  };

  var _createFilePicker = function _createFilePicker() {
  ...
  var getFileNames = function getFileNames(file) {
    return file.name;
  };

  var _createFilePicker2 = function createFilePicker() {
    var elem = this;
    loadMoxie();
    $(elem).on('mousedown.filereaderwaiting click.filereaderwaiting', false).parent().addClass('ws-loading');
    webshim.ready('moxie', function () {
      _createFilePicker2.call(elem);
    });
  };

  var noxhr = /^(?:script|jsonp)$/i;

  var notReadyYet = function notReadyYet() {
    loadMoxie();
  ...
  var testMoxie = function testMoxie(options) {
  ...
  var createMoxieTransport = function createMoxieTransport(options) {
    if (testMoxie(options)) {
      ...
      return {
        send: function send(headers, completeCallback) {
          var proressEvent = function proressEvent(obj, name) {
            if (options[name]) {
              var called = false;
  ...
  var transports = {
    ...
    moxie: function moxie(options, originalOptions, jqXHR) {
      if (testMoxie(options)) {
        loadMoxie(options);
        var ajax;
        var tmpTransport = {
  ...
  if (!document.createElement('canvas').toBlob) {
    webshim.defineNodeNameProperty('canvas', 'toBlob', {
      prop: {
        value: function value(cb, type, qualitiy) {
          var dataURL;
          var $canvas = $(this);
  ...
  webshim.ready('moxie', function () {
    ...
    _createFilePicker2 = _createFilePicker;
  ...
  webshim.addReady(function (context, contextElem) {
    $(context.querySelectorAll(sel)).add(contextElem.filter(sel)).each(_createFilePicker2);
  });
...
webshims.register('mediaelement-jaris', function ($, webshims, window, document, undefined, options) {
  ...
  var getSwfDataFromElem = function getSwfDataFromElem(elem) {
    try {
      elem.nodeName;
    } catch (er) {
      return null;
  ...
  var trigger = function trigger(elem, evt) {
    evt = $.Event(evt);
    evt.preventDefault();
  ...
  var setReadyState = function setReadyState(readyState, data) {
    if (readyState < 3) {
  ...
  var callSeeked = function callSeeked(data) {
  ...
  var onEvent = {
    onPlayPause: function onPlayPause(jaris, data, override) {
      var playing, type;
    ...
    onSeek: function onSeek(jaris, data) {
    ...
    onConnectionFailed: function onConnectionFailed(jaris, data) {
      mediaelement.setError(data._elem, 'flash connection error');
    },
    onNotBuffering: function onNotBuffering(jaris, data) {
      setReadyState(3, data);
    },
    onDataInitialized: function onDataInitialized(jaris, data) {
    ...
    onBuffering: function onBuffering(jaris, data) {
      if (data.ended) {
        data.ended = false;
      }
    ...
    onTimeUpdate: function onTimeUpdate(jaris, data) {
    ...
    onProgress: function onProgress(jaris, data) {
      if (data.ended) {
        data.ended = false;
      }
    ...
    onPlaybackFinished: function onPlaybackFinished(jaris, data) {
    ...
    onVolumeChange: function onVolumeChange(jaris, data) {
  ...
  var workActionQueue = function workActionQueue(data) {
  ...
  if (hasNative) {
    ...
    var hidePlayerEvents = function hidePlayerEvents(event) {
    ...
    addMediaToStopEvents = function addMediaToStopEvents(elem) {
  ...
  var getComputedDimension = function () {
    ...
    var getVideoDims = function getVideoDims(data) {
      var ret, poster, img;
      ...
      return ret || {
        width: 300,
        height: data._elemNodeName == 'video' ? 150 : 50
      };
    };

    var getCssStyle = function getCssStyle(elem, style) {
    ...
    var addMinMax = function addMinMax(elem, ret) {
      var i, prop;
      var hasMinMax = false;
  ...
  var setElementDimension = function setElementDimension(data, hasControls) {
    var dims;
  ...
  var regs = {
    ...
    q: /\?/g
      ...
      replaceVar = function replaceVar(val) {
  ...
  mediaelement.createSWF = function (elem, canPlaySrc, data) {
    ...
    if (data && data.swfCreated) {
      ...
      box = $('<div class="polyfill-' + elemNodeName + ' polyfill-mediaelement ' + webshims.shadowClass + '" id="wrapper-' + elemId + '"><div id="' + elemId + '"></div>').css({
        position: 'relative',
        overflow: 'hidden'
      });
  ...
  var createSwf = function createSwf(elem, canPlaySrc, data, elemId, hasControls, elemNodeName) {
  ...
  var queueSwfMethod = function queueSwfMethod(elem, fn, args, data) {
  ...
  ['audio', 'video'].forEach(function (nodeName) {
    ...
    var createGetProp = function createGetProp(key) {
    ...
    var createGetSetProp = function createGetSetProp(key, setFn) {
      createGetProp(key);
  ...
  var addCanvasBridge = function addCanvasBridge() {
    ...
    var addToBlob = function addToBlob() {
    ...
    CanvasRenderingContext2D.prototype.drawImage = function (elem) {
      ...
      if (isVideo[elem.nodeName] && (data = webshims.data(elem, 'mediaelement')) && data.isActive == 'third' && data.api.api_image) {
        ...
        img = document.createElement('img'); //todo find a performant sync way
  ...
  if (hasNative && hasFlash && !options.preferFlash) {
    ...
    var switchOptions = function switchOptions(e) {
      var media, error, parent;

      if (($(e.target).is('audio, video') || (parent = e.target.parentNode) && $('source', parent).last()[0] == e.target) && (media = $(e.target).closest('audio, video')) && !media.hasClass('nonnative-api-active')) {

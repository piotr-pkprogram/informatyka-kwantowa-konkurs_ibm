...
webshims.register('mediacontrols', function ($, webshims, window) {
  ...
  var loadLazy = function loadLazy() {
    if (!loadLazy.loaded) {
  ...
  var lazyLoadPlugin = function lazyLoadPlugin(fn) {
    if (!fn) {
      fn = '_create';
    }

    var rfn = function rfn(c, media) {
      var obj = this;
      var args = arguments;
      loadLazy();
  ...
  jme.registerPlugin('captions', {
    ...
    _create: function _create(control, media, base) {
      ...
      control.wsclonedcheckbox = $(control).clone().attr({
        role: 'checkbox'

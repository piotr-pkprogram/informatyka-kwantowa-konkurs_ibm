...
(function ($) {
  ...
  var domWrite = function domWrite() {
    setTimeout(function () {
  ...
  $.extend(navigator.geolocation, function () {
    var pos;
    var api = {
      getCurrentPosition: function getCurrentPosition(success, error, opts) {
        var locationAPIs = 2,
            errorTimer,
            googleTimer,
            calledEnd,
          ...
          if (pos) {
            ...
            googleCallback = function googleCallback() {
          locationAPIs--;
          getGoogleCoords();
          endCallback();
        },
          ...
          clearTimeout(errorTimer);
            ...
            getGoogleCoords = function getGoogleCoords() {
          ...
          return true;
            ...
            getInitCoords = function getInitCoords() {
          if (pos) {
            return;
          }
...
webshims.register('details', function ($, webshims, window, doc, undefined, options) {
  var isInterActiveSummary = function isInterActiveSummary(summary) {
  ...
  var bindDetailsSummary = function bindDetailsSummary(summary, details) {
    summary = $(summary);
    details = $(details);
    ...
    if (!oldSummary || summary[0] !== oldSummary[0]) {
      if (oldSummary) {
        if (oldSummary.hasClass('fallback-summary')) {
          ...
          oldSummary.off('.summaryPolyfill').removeData('detailsElement').removeAttr('role').removeAttr('tabindex').removeAttr('aria-expanded').removeClass('summary-button').find('span.details-open-indicator').remove();
        }
      }
  ...
  var getSummary = function getSummary(details) {
  ...
  webshims.createElement('summary', function () {
    ...
    $(this).on({
      ...
      'click.summaryPolyfill': function clickSummaryPolyfill(e) {
        var details = isInterActiveSummary(this);
      ...
      'keydown.summaryPolyfill': function keydownSummaryPolyfill(e) {
...
webshims.register('track', function ($, webshims, window, document, undefined) {
  ...
  var mediaelement = webshims.mediaelement;
  var id = new Date().getTime(); //descriptions are not really shown, but they are inserted into the dom
  ...
  var createEventTarget = function createEventTarget(obj) {
    var eventList = {};
  ...
  var updateMediaTrackList = function updateMediaTrackList(baseData, trackList) {
    var i, len;
    ...
    if (removed.length || added.length) {
      ...
      for (i = 0, len = removed.length; i < len; i++) {
        $([trackList]).triggerHandler($.Event({
          type: 'removetrack',
          track: removed[i]
        }));
      }
  ...
  var refreshTrack = function refreshTrack(track, trackData) {
    if (!trackData) {
  ...
  var isDefaultTrack = function () {
    ...
    return function (track) {
      ...
      if (isDefault && (kind = $.prop(track, 'kind')) != 'metadata') {
        firstDefaultTrack = $(track).parent().find('track[default]').filter(function () {
  ...
  mediaelement.parseCueTextToHTML = function () {
    ...
    var addToTemplate = function addToTemplate(localName, attribute, tag, html) {
      var ret;
    ...
    var replacer = function replacer(html) {
  ...
  var mapTtmlToVtt = function mapTtmlToVtt(i) {
    var content = i + '';
  ...
  var ttmlTextToVTT = function ttmlTextToVTT(ttml) {
    ttml = $.parseXML(ttml) || [];
  ...
  */
  ...
  mediaelement.parseCaptionChunk = function () {
    ...
    return function (subtitleElement, objectCount) {
      ...
      var timestampMatch, tmpCue; // WebVTT Special Cue Logic
      ...
      subtitleParts = subtitleElement.split(/\n/g); // Trim off any blank lines (logically, should only be max. one, but loop to be sure)
  ...
  mediaelement.parseCaptions = function (captionData, track, complete) {
    var cue, _lazyProcess, regWevVTT, startDate, isWEBVTT;
    ...
    if (captionData) {
      ...
      _lazyProcess = function lazyProcess(i, len) {
        for (; i < len; i++) {
          cue = captionData[i];
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          _lazyProcess(0, captionData.length);
        }, 9);
      }, 9);
    } else {
  ...
  webshims.defineNodeNamesProperties(['track'], {
    ...
    readyState: {
      get: function get() {
        return (webshims.data(this, 'trackData') || {
          readyState: 0
        }).readyState;
      },
      writeable: false
  ...
  webshims.defineNodeNamesProperties(['audio', 'video'], {
    ...
    addTextTrack: {
      value: function value(kind, label, lang) {
  ...
  var thUpdateList = function thUpdateList(e) {
  ...
  var getNativeReadyState = function getNativeReadyState(trackElem, textTrack) {
    return textTrack.readyState || trackElem.readyState;
  };

  var stopOriginalEvent = function stopOriginalEvent(e) {
    if (e.originalEvent) {
      e.stopImmediatePropagation();
    }
  };

  var hideNativeTracks = function hideNativeTracks() {
  ...
  webshims.addReady(function (context, insertedElement) {
    ...
    $('video, audio', context).add(insertedMedia).each(function () {
      updateMediaTrackList.call(this);
    }).on('emptied updatetracklist wsmediareload', thUpdateList).each(function () {
      if (support.track) {

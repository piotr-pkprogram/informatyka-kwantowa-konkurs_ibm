...
(function (window, document, undefined) {
  ...
  var lowTreshHold, partialLowTreshHold, isLandscape, lazyFactor, tMemory, substractCurRes, warn, eminpx, alwaysCheckWDescriptor, resizeThrottle, evalID; // local object for method references and testing exposure
  ...
  var isSSL = location.protocol == "https:"; // baseStyle also used by getEmValue (i.e.: width: 1em is important)
  ...
  var off = function off(obj, evt, fn, capture) {
  ...
  var evalCSS = function () {
    ...
    var buidlStr = memoize(function (css) {
      ...
      /\band\b/g, "&&", // interpret `,`
      /,/g, "||", // interpret `min-` as >=
      /min-([a-z-\s]+):/g, "e.$1>=", // interpret `min-` as <=
  ...
  var respimage = function respimage(opt) {
    var elements, i, plen;
    var options = opt || {};
  ...
  var parseDescriptor = memoize(function (descriptor) {
    ...
    if (parsedDescriptor) {
      ...
      if (parsedDescriptor.match(regDescriptor)) {
        ...
        if (RIDEBUG && (descriptorObj[0] < 0 || isNaN(descriptorObj[0]) || descriptorObj[1] == "w" && /\./.test('' + descriptorObj[0]))) {
  ...
  types["image/png"] = true; // test svg support
  ...
  ri.ns = ("ri" + new Date().getTime()).substr(0, 9); // srcset support test

  ri.supSrcset = "srcset" in image;
  ri.supSizes = "sizes" in image; // using ri.qsa instead of dom traversing does scale much better,
  ...
  ri.DPR = DPR || 1;
  ri.u = units; // container of supported mime types that one might need to qualify before using
    ...
    if (!set.cands) {
      ...
      while (srcset) {
        ...
        if (pos != -1) {
          ...
          last = url.charAt(url.length - 1); // 6. If url ends with a U+002C COMMA character (,), remove that character from url
  ...
  ri.calcListLength = function (sourceSizeListStr) {
    ...
    if (!(sourceSizeListStr in sizeLengthCache) || cfg.uT) {
      ...
      for (i = 0, len = sourceSizeList.length; i < len; i++) {
        ...
        sourceSize = sourceSizeList[i]; // Split "( min-width: 50em ) 100%" into separate strings
  ...
  ri.applySetCandidate = function (candidates, img) {
    ...
    var candidate, dpr, i, j, diff, length, bestCandidate, curSrc, curCan, isSameSet, candidateSrc, abortCurSrc, oldRes;
    ...
    dpr = ri.DPR;
    oldRes = curCan && curCan.res; //if we have a current source, we might either become lazy or give this source some advantage
    ...
    if (!bestCandidate) {
      ...
      for (i = 0; i < length; i++) {
        ...
        if (candidate.res >= dpr) {
          ...
          if (candidates[j] && (diff = candidate.res - dpr) && (abortCurSrc || curSrc != ri.makeUrl(candidate.url)) && chooseLowRes(candidates[j].res, diff, dpr)) {
  ...
  ri.setSrc = function (img, bestCandidate) {
    ...
    if (reflowBug) {
      ...
      img.style.zoom = "0.999"; // next line only should trigger a repaint
  ...
  ri.fillImg = function (element, options) {
    ...
    imageData = element[ri.ns]; // if the element has already been evaluated, skip it
  ...
  ri.setupRun = function (options) {
    if (!alreadyRun || isVwDirty || DPR != window.devicePixelRatio) {
      updateMetrics(); // if all images are reevaluated clear the resizetimer
  ...
  if (window.HTMLPictureElement) {
    ...
    (function () {
      ...
      var resizeEval = function resizeEval() {
        ri.fillImgs();
      };
  ...
  ri.respimage = respimage; //use this internally for easy monkey patching/performance testing
  ...
  if (RIDEBUG) {
    ...
    var testImgDimensions = function testImgDimensions(img, candidate) {
      var onload = function onload() {
        ...
        if (imgWidth && canWidth) {
          ...
          if (Math.abs(imgWidth - canWidth) > 50) {
            ...
            } else if (candidate.x && dif < xtreshhold) {//warn("Image too much resized. Image was shown with "+ imgWidth +" but has a normalized width of "+ canWidth +". Maybe you should use a w descriptor instead of an x descriptor. img: "+ candidate.url);
            }
          }
        }
    ...
    var testMediaOrder = function () {
      ...
      var checkSetOrder = function checkSetOrder(set, sets, index, type) {
        var i, curSet;
      ...
      var mediaTest = function mediaTest(sets, type) {
...
(function (factory) {
  ...
  var setSize = function setSize(width, img, data) {
  ...
  var loadBg = function loadBg(url, img, data) {
...
(function (factory) {
  ...
  var setTypeValue = function setTypeValue(types, value) {
    var i;

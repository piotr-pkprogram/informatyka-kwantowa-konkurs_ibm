...
webshims.register('form-combat', function ($, webshims) {
  ...
  var addReplacement = function addReplacement(pName, dataName, obj) {
    if ($.fn[pName]) {
      if (_typeof(dataName) == 'object') {
        obj = dataName;
        dataName = pName;
      }
  ...
  if ($.fn.select2) {
    addReplacement('select2', {
      ...
      _create: function _create(elem, shadow, shadowFocus, widgetData) {
        var onValidate;
  ...
  var uiSelect = {
    shadow: 'button',
    shadowFocus: function shadowFocus(data) {
  ...
  find.register = function (elem, data, pluginDescriptor, plugin) {
    ...
    if (shadow && (replacementDatas.success || $(shadowFocus).attr('tabindex') || $(shadowFocus).prop('tabIndex') > -1)) {
      webshims.addShadowDom(elem, shadow, {
        shadowFocusElement: shadowFocus
      });

...
(function ($, version) {
  ...
  var mPrecision = function mPrecision(value, precision) {
    if (precision === undefined) precision = 0;
    return Math.round(value * Math.pow(10, precision)) / Math.pow(10, precision);
  };

  var elemExtend = function elemExtend(deep, elem, data) {
    var ex = elem;
  ...
  function Slider(bar, options) {
    ...
    minX = 0,
        ...
        fireChangeEvents = function fireChangeEvents(context) {
    ...
    function setValuesFromMousePosition(e) {
      ...
      barH = bar.h; // keep the arrow within the bounds of the bar
      ...
      drawTimeout = setTimeout(function () {
        ...
        val = function val(name, value, context) {
      ...
      changed && fireChangeEvents.call($this, context || $this);
        ...
        range = function range(name, value) {
      var set = value !== undefined;
  ...
  function ColorValuePicker(picker, color, bindedHex, alphaPrecision) {
    ...
    function setValueInRange(value, min, max) {
      ...
      return value;
        ...
        colorChanged = function colorChanged(ui, context) {
      var all = ui.val('all');
  ...
  $.wsjPicker = {
    ...
    function Color(init) {
      ...
      if (init) {
        if (init.ahex != null) val('ahex', init);else if (init.hex != null) val((init.a != null ? 'a' : '') + 'hex', init.a != null ? {
          ahex: init.hex + ColorMethods.intToHex(init.a)
        } : init);else if (init.r != null && init.g != null && init.b != null) val('rgb' + (init.a != null ? 'a' : ''), init);else if (init.h != null && init.s != null && init.v != null) val('hsv' + (init.a != null ? 'a' : ''), init);
      }
    },
    ...
    return this.each(function () {
      ...
      function positionMapAndBarArrows(ui, context) {
        ...
        if (context != colorBar) {
          ...
          updatePreview = function updatePreview(ui) {
        try {
          var all = ui.val('all');
          activePreview.css({
          ...
          updateMapVisuals = function updateMapVisuals(ui) {
        switch (settings.color.mode) {
          case 'h':
        ...
        setAlpha.call($this, colorMapL3, mPrecision((255 - (a || 0)) * 100 / 255, 4));
          ...
          updateBarVisuals = function updateBarVisuals(ui) {
        switch (settings.color.mode) {
          case 'h':
            var a = ui.val('a');
          ...
          setBG = function setBG(el, c) {
        el.css({
          backgroundColor: c && c.length == 6 && '#' + c || 'transparent'
        });
      },
          setImg = function setImg(img, src) {
        img.css({
          backgroundImage: 'url(\'' + src + '\')'
        });
      },
          setImgLoc = function setImgLoc(img, y) {
        img.css({
          top: y + 'px'
        });
      },
          setAlpha = function setAlpha(obj, alpha) {
        obj.css({
      ...
      function commitColor() {
        ...
        $.isFunction(commitCallback) && commitCallback.call($this, color.active, okButton);
          ...
          currentColorChanged = function currentColorChanged(ui, context) {
        var hex = ui.val('hex');
        currentPreview.css({
        ...
        setAlpha.call($this, currentPreview, mPrecision((ui.val('a') || 0) * 100 / 255, 4));
          ...
          moveBarMouseDown = function moveBarMouseDown(e) {
        ...
        return false;
          ...
          documentMouseUp = function documentMouseUp(e) {
        $(document).off('mousemove', documentMouseMove).off('mouseup', documentMouseUp);
        return false;
      },
          quickPickClicked = function quickPickClicked(e) {
        ...
        container = $($this);
        container.html(controlHtml); // initialize the objects to the source code just injected
        ...
        colorBarL6 = BarMaps.filter('.Map6').eq(0); // create color pickers and maps
        ...
        tbody.find('td.Radio input').on('click', radioClicked); // initialize quick list
      ...
      elemExtend(false, $this, // public properties, methods, and callbacks
      {
        ...
        color: color,
        setColorMode: function setColorMode(val) {
...
webshims.register('color-picker', function ($, webshims, window, document, undefined, options) {
  ...
  picker.color.showPickerContent = function () {
    ...
    var jPickerApi;
    var fns = {
      setPicker: function setPicker(data) {
      ...
      setInputColor: function setInputColor(data) {
        var oldAlpha;
    ...
    var pickerSet = function pickerSet(fn, data, value) {
      if (data == curData && fns[fn]) {
        fns[fn](data);
      }
    };

    var createPicker = function createPicker(data) {
    ...
    var implementPickerFor = function implementPickerFor(data) {
      createPicker();

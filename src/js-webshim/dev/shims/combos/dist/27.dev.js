...
webshims.register('form-shim-extend2', function ($, webshims, window, document, undefined, options) {
  ...
  var support = webshims.support; //support getSetAttribute

  var supportGetSetAttribute = !('getSetAttribute' in $.support && !$.support.getSetAttribute); //submitbubbles for IE6-IE8

  var supportSubmitBubbles = !('submitBubbles' in $.support) || $.support.submitBubbles;

  var addSubmitBubbles = function addSubmitBubbles(form) {
    if (!supportSubmitBubbles && form && _typeof(form) == 'object' && !form._submit_attached) {
  ...
  if (!('maxLength' in document.createElement('textarea'))) {
    var constrainMaxLength = function () {
      ...
      var constrainLength = function constrainLength() {
      ...
      return function (element, maxLength) {
        ...
        if (maxLength > -1) {
          ...
          lastElement = $(element);
          lastElement.on({
            'keydown.maxlengthconstraint keypress.maxlengthconstraint paste.maxlengthconstraint cut.maxlengthconstraint': function keydownMaxlengthconstraintKeypressMaxlengthconstraintPasteMaxlengthconstraintCutMaxlengthconstraint(e) {
  ...
  if (!support.fieldsetdisabled || !support.fieldsetelements) {
    (function () {
      if (!support.fieldsetdisabled) {
        ...
        var isDisabledChild = function isDisabledChild(elem) {
        ...
        var isDisabledChildFilter = hasIsDisabled ? function () {
          return this.isDisabled && $.find.matchesSelector(this, 'fieldset[disabled] *');
        } : 'fieldset[disabled] *';
        var isEnableChildFilter = hasIsDisabled ? function () {
          return !this.isDisabled && !$.find.matchesSelector(this, 'fieldset[disabled] *');
        ...
        webshims.addReady(function (context) {
          $(context).filter(isDisabledChildFilter).find(disableElementsSel).each(groupControl.disable);
        });
      }
      ...
      var removeAddedElements = function removeAddedElements(form) {
      ...
      var getAssociatedForm = function getAssociatedForm() {
        ...
        $(document).on('click', function (e) {
          if (submitters[e.target.type] && !e.isDefaultPrevented() && webshims.contentAttr(e.target, 'form')) {
            ...
            if (trueForm && trueForm != formIn) {
              clone = $(e.target).clone().removeAttr('form').addClass('webshims-visual-hide').on('click', stopPropagation).appendTo(trueForm);
      ...
      if (!$.fn.finish && parseFloat($.fn.jquery, 10) < 1.9) {
        ...
        $.fn.serializeArray = function () {
          return this.map(function () {
            ...
            return this.name && !$(this).is(':disabled') && (this.checked || rselectTextarea.test(this.nodeName) || rinput.test(this.type));
          }).map(function (i, elem) {
            var val = $(this).val();
  ...
  if (!('value' in document.createElement('progress'))) {
    (function () {
      ...
      var updateProgress = function updateProgress(progress) {
        ...
        $('> span', progress).css({
          width: (position < 0 ? 100 : position * 100) + '%'
        });
      };
      ...
      var desc = {
        position: {
          prop: {
            get: function get() {
              var max; //jQuery 1.8.x try's to normalize "0" to 0
      ...
      webshims.createElement('progress', function () {
        var labels = $(this).attr({
          role: 'progressbar',
          'aria-valuemin': '0'
  ...
  if (!('setSelectionRange' in document.createElement('input'))) {
    (function () {
      var getSelection = function getSelection(elem, getStart) {
      ...
      ['input', 'textarea'].forEach(function (name) {
        var isTextarea = name == 'textarea'; //email?
        ...
        webshims.defineNodeNameProperties(name, {
          ...
          setSelectionRange: {
            value: function value(start, end, dir) {
              if (this.createTextRange && (isTextarea || allowedTypes[$.prop(this, 'type')])) {
                var range = this.createTextRange();
                range.collapse(true);
                range.moveStart('character', start);
  ...
  (function () {
      ...
      (function () {
        ...
        if (ua.indexOf('Mobile') != -1 && ua.indexOf('Safari') != -1) {
          $(window).on('orientationchange', function () {
            ...
            var retVal = function retVal(i, value) {
              return value;
            };
    ...
    var setSelection = function setSelection(elem) {
      try {
    ...
    var hidePlaceholder = function hidePlaceholder(elem, data, value, _onFocus) {
      if (value === false) {
        value = $.prop(elem, 'value');
      }
      ...
      if (!isOver && elem.type != 'password') {
        if (!value && _onFocus && setSelection(elem)) {
          ...
          $(elem).off('.placeholderremove').on({
            'keydown.placeholderremove keypress.placeholderremove paste.placeholderremove input.placeholderremove': function keydownPlaceholderremoveKeypressPlaceholderremovePastePlaceholderremoveInputPlaceholderremove(e) {
            ...
            'mousedown.placeholderremove drag.placeholderremove select.placeholderremove': function mousedownPlaceholderremoveDragPlaceholderremoveSelectPlaceholderremove(e) {
              setSelection(elem);
              clearTimeout(selectTimer);
          ...
          return;
        } else if (!_onFocus && !value && elem.value) {
          //especially on submit
          elem.value = value;
        }
      ...
      data.box.removeClass('placeholder-visible');
        ...
        showPlaceholder = function showPlaceholder(elem, data, placeholderTxt) {
      ...
      data.box.addClass('placeholder-visible');
        ...
        changePlaceholderVisibility = function changePlaceholderVisibility(elem, value, placeholderTxt, data, type) {
      if (!data) {
      ...
      return {
        create: function create(elem) {
          ...
          if (elem.type == 'password' || isOver) {
            ...
            if (isResponsive || $(elem).hasClass('responsive-width') || (elem.currentStyle || {
              width: ''
            }).width.indexOf('%') != -1) {
              data.box = data.text;
            } else {
              data.box = $('<span class="placeholder-box placeholder-box-' + (elem.nodeName || '').toLowerCase() + ' placeholder-box-' + $.css(elem, 'float') + '" />').insertAfter(elem);
              data.box.append(elem);
            }

            data.text.insertAfter(elem).on('mousedown.placeholder', function () {
            ...
            $(elem).onWSOff('updateshadowdom', function () {
        ...
        update: function update(elem, val) {
  ...
  (function () {
    ...
    var outputCreate = function outputCreate(elem) {
    ...
    (function () {
      ...
      var observe = function observe(newInput) {
        unbind();
        input = newInput;
      ...
      $(doc).on('focusin wswidgetfocusin', function (e) {
